...
class SettingsWindow(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.languages_list.itemClicked.connect(
            lambda item: self.language_edit.setText(item.text())
        )
        ...
        self.delete_prompt_btn.setIcon(
            self.style().standardIcon(self.style().SP_TrashIcon)
        )
        ...
        self.languages_list.keyPressEvent = lambda event: self.handle_list_key_event(
            event, self.languages_list
        )
        self.models_list.keyPressEvent = lambda event: self.handle_list_key_event(
            event, self.models_list
        )
        self.prompts_list.keyPressEvent = lambda event: self.handle_list_key_event(
            event, self.prompts_list
        )
    ...
    def load_settings(self):
        ...
        theme_mode = self.settings_manager.get_theme()
        theme_index = {"system": 0, "light": 1, "dark": 2}.get(theme_mode, 0)
    ...
    def save_settings(self):
        ...
        theme_mode = {0: "system", 1: "light", 2: "dark"}.get(
            self.theme_combo.currentIndex(), "system"
        )
        ...
        if selected_item:
            ...
            if len(parts) >= 2:
                ...
                if self.parent():
                    index = self.parent().model_combo.findText(display_name)
                    if index >= 0:
    ...
    def remove_model(self):
        ...
        parts = display_name.split(" - ")
        if len(parts) < 2:
            return  # Некорректный формат имени
        ...
        reply = QMessageBox.question(
            ...
            f"Вы уверены, что хотите удалить модель '{display_name}'?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )

        if reply == QMessageBox.Yes:
            self.settings_manager.remove_model(provider, model_name)
            self.load_settings()
            if self.parent():
                self.parent().update_model_combo()
    ...
    def edit_model(self):
        ...
        if selected:
            ...
            current_model = next(
                (model for model in available_models if model["name"] == display_name),
                None,
            )

            if current_model:
                dialog = AddModelDialog(self, edit_model_data=current_model)
                dialog.center_relative_to_parent()
                dialog.apply_theme()
                ...
                if dialog.exec_() == QDialog.Accepted:
                    model_data = dialog.get_model_info()
                    if model_data:
                        # Удаляем старую модель и добавляем новую
                        self.settings_manager.remove_model(
                            current_model["provider"], current_model["model_name"]
                        )
                        self.settings_manager.add_model(
                            provider=model_data["provider"],
                            model_name=model_data["model_name"],
                        ...
                        # Это может быть неточно, если изменился порядок
                        new_list = [
                            self.models_list.item(i).text()
                            for i in range(self.models_list.count())
                        ]
                        new_name = (
                            f"{model_data['model_name']} - {model_data['provider']}"
                        )
                        if new_name in new_list:
                            self.models_list.setCurrentRow(new_list.index(new_name))
    ...
    def add_prompt(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            prompt_info = dialog.get_prompt_info()
            if prompt_info:
                self.settings_manager.add_prompt(
